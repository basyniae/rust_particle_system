...
use crate::solver::ips_rules::{ips_rules_constructor, IPSKind};
use crate::visualization::{coloration_constructor, save_as_gif, save_as_growth_img};
...
pub fn contact_process_img() {
    ...
    let ips_rules = ips_rules_constructor(IPSKind::ContactProcess, vec![1.1, 0.4]);
    let ips_coloration = coloration_constructor(IPSKind::ContactProcess, vec![0.1, 1.4]);
...
pub fn contact_process_img_random_initial() {
    ...
    // );
    //
    // let ips_rules = ips_rules_constructor(IPSKind::ContactProcess, vec![0.1, 1.4]).deref();
    //
    // let solution = particle_system_solver(
    //     ips_rules,
    //     GridND::from(vec![nr_particles]),
    ...
    // // println!("Solution: {:?}", solution);
    //
    // save_as_growth_img(ips_rules, solution, "contact_process_specimen_rnd_initial.png", nr_particles as u32)
}
